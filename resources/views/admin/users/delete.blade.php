...
@section('content')
  ...
  <div class="container-fluid">
    <form action="{{url('/admin/users/'.$usuario->id)}}" method="POST" class="mx auto">
      @csrf
      @method('DELETE')
      <h4 class=text-center>¿Estás seguro que deseas eliminar este registro?</h4>
      <div class="mb-3 mt-5">
        <label for="" class="form-label">Nombre de usuario</label>
        <input type="text" name="name" value="{{$usuario->name}}" class="form-control" aria-describedby="emailHelp" disabled>
      </div>
      <div class="mb-3">
